...
namespace rock::contact_activity_tracker
{
     ...
     * this body recently"; this tracker answers the separate continuity
    ...
    public:
        ...
        {
            return isHandContactFresh(isLeft, handBodyId, targetBodyId, kActiveContactFrames);
        }

        bool isHandContactFresh(
            bool isLeft,
            std::uint32_t handBodyId,
            std::uint32_t targetBodyId,
            std::uint32_t maxAgeFrames) const
        {
            if (!isTrackableBodyPair(handBodyId, targetBodyId)) {
                return false;
            }
            ...
            const auto key = makeBodyPairKey(handBodyId, targetBodyId);
            const auto* slot = findSlot(isLeft ? _left : _right, key);
            if (!slot) {
                return false;
            }
            ...
            for (int attempt = 0; attempt < 3; ++attempt) {
                const auto keyBefore = slot->key.load(std::memory_order_acquire);
                if (keyBefore != key) {
                    return false;
                }
                const auto lastFrame = slot->lastFrame.load(std::memory_order_acquire);
                const auto keyAfter = slot->key.load(std::memory_order_acquire);
                if (keyBefore == keyAfter) {
                    return frameAge(frame, lastFrame) <= maxAgeFrames;
                }
            }
            return false;
        }
        ...
        /*
         * A generated hand has several collider bodies, and one collider can
         * contact more than one nearby weapon body during the same physics
         * step. Query by target across the whole hand bank so an exact bolt
         * contact cannot be hidden by a later barrel contact ("last wins").
         */
        bool hasFreshHandContactWithTarget(
            bool isLeft,
            std::uint32_t targetBodyId,
            std::uint32_t maxAgeFrames) const
        {
            if (!isTrackableBodyId(targetBodyId)) {
                return false;
            }
            ...
            for (const auto& slot : bank.slots) {
                for (int attempt = 0; attempt < 3; ++attempt) {
                    const auto keyBefore = slot.key.load(std::memory_order_acquire);
                    if (keyBefore == kFreeContactActivityKey || keyBefore == kReservedContactActivityKey) {
                        break;
                    }
                    const auto storedTargetBodyId = slot.targetBodyId.load(std::memory_order_acquire);
                    const auto lastFrame = slot.lastFrame.load(std::memory_order_acquire);
                    const auto keyAfter = slot.key.load(std::memory_order_acquire);
                    if (keyBefore != keyAfter) {
                        continue;
                    }
                    if (storedTargetBodyId == targetBodyId && frameAge(frame, lastFrame) <= maxAgeFrames) {
                        return true;
                    }
                    break;
                }
