...
namespace rock::collision_suppression_registry
{
    namespace
    {
        ...
        const char* ownerName(CollisionSuppressionOwner owner)
        {
            ...
            case CollisionSuppressionOwner::HostExternal:
                return "HostExternal";
            }
            return "Unknown";
        }
    ...
    RuntimeSuppressionResult CollisionSuppressionRegistry::acquire(
        RE::hknpWorld* world,
        std::uint32_t bodyId,
        CollisionSuppressionOwner owner,
        const char* context,
        RuntimeSuppressionLogMode logMode)
    {
        RuntimeSuppressionResult result{};
        result.bodyId = bodyId;
        const bool logDetails = logMode == RuntimeSuppressionLogMode::Detailed;
        ...
        if (logDetails && (result.firstLeaseForBody || result.filterChanged)) {
            ROCK_LOG_DEBUG(Hand,
    ...
    RuntimeSuppressionResult CollisionSuppressionRegistry::release(
        RE::hknpWorld* world,
        std::uint32_t bodyId,
        CollisionSuppressionOwner owner,
        const char* context,
