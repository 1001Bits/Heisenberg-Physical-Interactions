...
namespace rock::performance_profiler
{
    namespace
    {
        ...
        /*
         * 0.1 ms buckets retain useful resolution around the 11.1/13.9/22.2 ms
         * VR pacing boundaries. The final bucket is an overflow bucket; exact
         * total and maximum ticks are tracked separately, so averages and max
         * remain exact even for a long hitch.
         */
        inline constexpr std::uint64_t kBenchmarkHistogramBinMicroseconds = 100;
        inline constexpr std::uint64_t kBenchmarkHistogramRangeMicroseconds = 500000;
        inline constexpr std::size_t kBenchmarkHistogramRegularBins =
            static_cast<std::size_t>(
                kBenchmarkHistogramRangeMicroseconds /
                kBenchmarkHistogramBinMicroseconds);
        inline constexpr std::size_t kBenchmarkHistogramBinCount =
            kBenchmarkHistogramRegularBins + 1;

        struct BenchmarkAccum
        {
            std::array<std::uint32_t, kBenchmarkHistogramBinCount>
                histogram{};
            std::uint64_t previousBoundaryTicks{ 0 };
            std::uint64_t totalTicks{ 0 };
            std::uint64_t maxTicks{ 0 };
            std::uint64_t samples{ 0 };
            std::uint32_t warmupRemaining{ 0 };
            bool firstEligibleNoticeQueued{ false };
        };
        ...
        Settings s_settings;
        BenchmarkSettings s_benchmarkSettings;
        BenchmarkAccum s_benchmarkAccum;
        ...
        constexpr int sanitizeBenchmarkMode(int value) noexcept
        {
            return std::clamp(value, 0, 2);
        }

        constexpr const char* benchmarkModeName(int mode) noexcept
        {
            switch (mode) {
            case 1:
                return "A_FULL";
            case 2:
                return "B_ROCK_BYPASSED";
            default:
                return "OFF";
            }
        }
        ...
        constexpr const char* scopeName(Scope scope) noexcept
        {
            ...
            case Scope::FrameUpdate:
                return "frame";
            case Scope::PhysicsInteractionUpdate:
                return "physicsInteractionUpdate";
            ...
            case Scope::CharacterControllerContactFilter:
                return "characterControllerContactFilter";
            ...
            case Scope::TwoHandedGripFrame:
                return "twoHandedGripFrame";
            ...
            case Scope::HostHandAuthorityApply:
                return "hostHandAuthorityApply";
            case Scope::LegacyFingerPoseApply:
                return "legacyFingerPoseApply";
            case Scope::PipboyFrame:
                return "pipboyFrame";
            case Scope::PipboyDeckContactQuery:
                return "pipboyDeckContactQuery";
        ...
        std::uint64_t ticksToMicroseconds(std::uint64_t ticks) noexcept
        {
            if (!ensureFrequencyReady()) {
                return 0;
            }
            ...
            const auto frequency =
                static_cast<std::uint64_t>(s_frequency.QuadPart);
            const auto wholeSeconds = ticks / frequency;
            const auto remainder = ticks % frequency;
            return wholeSeconds * 1000000u +
                   (remainder * 1000000u) / frequency;
        }

        void clearBenchmarkMeasurement() noexcept
        {
            s_benchmarkAccum.histogram.fill(0);
            s_benchmarkAccum.totalTicks = 0;
            s_benchmarkAccum.maxTicks = 0;
            s_benchmarkAccum.samples = 0;
        }

        void resetBenchmarkAccum(std::uint32_t warmupFrames) noexcept
        {
            s_benchmarkAccum = {};
            s_benchmarkAccum.warmupRemaining = warmupFrames;
        }
        ...
        struct QueuedSnapshot
        {
            ...
            int benchmarkNoticeMode{ 0 };
            std::uint32_t benchmarkNoticeIntervalFrames{ 0 };
            std::uint32_t benchmarkNoticeWarmupFrames{ 0 };
            bool benchmarkArmedNotice{ false };
            bool benchmarkFirstEligibleNotice{ false };
        };
        ...
        private:
            ...
            void writeSnapshot(const QueuedSnapshot& snapshot) noexcept
            {
                try {
                    ...
                    bool flushNoticeImmediately = false;
                    if (snapshot.benchmarkArmedNotice) {
                        logger->info(
                            "[ROCK::Performance] Benchmark armed mode={}({}) warmupFrames={} intervalFrames={}; this confirms the Heisenberg DLL loaded and recognized the benchmark config",
                            snapshot.benchmarkNoticeMode,
                            benchmarkModeName(
                                snapshot.benchmarkNoticeMode),
                            snapshot.benchmarkNoticeWarmupFrames,
                            snapshot.benchmarkNoticeIntervalFrames);
                        flushNoticeImmediately = true;
                    }
                    if (snapshot.benchmarkFirstEligibleNotice) {
                        logger->info(
                            "[ROCK::Performance] Benchmark mode={}({}) received its first eligible gameplay frame; warmup/measurement is now advancing",
                            snapshot.benchmarkNoticeMode,
                            benchmarkModeName(
                                snapshot.benchmarkNoticeMode));
                        flushNoticeImmediately = true;
                    ...
                    if (snapshot.detailedProfile) {
                        ...
                        // Rank accumulated CPU time on the writer thread. A
                        // scope can run more than once per frame or on a
                        // physics callback, so aggregateMsPerFrame is the most
                        // useful first-pass outlier signal; avgMs remains the
                        // average duration of one invocation above. Nested
                        // scopes intentionally remain visible and must not be
                        // summed together as exclusive wall time.
                        std::array<
                            const ScopeSnapshot*,
                            static_cast<std::size_t>(Scope::Count)>
                            rankedScopes{};
                        std::size_t rankedScopeCount = 0;
                        for (const auto& item : snapshot.scopes) {
                            // The two umbrella timers are useful sanity checks
                            // but are not independently actionable components;
                            // keep them in the full listing without consuming
                            // a TOP5 subsystem slot.
                            if (item.scope == Scope::FrameUpdate ||
                                item.scope ==
                                    Scope::PhysicsInteractionUpdate ||
                                item.totalTicks == 0) {
                                continue;
                            }
                            rankedScopes[rankedScopeCount++] = &item;
                        }
                        std::sort(
                            rankedScopes.begin(),
                            rankedScopes.begin() +
                                static_cast<std::ptrdiff_t>(
                                    rankedScopeCount),
                            [](const ScopeSnapshot* left,
                               const ScopeSnapshot* right) {
                                return left->totalTicks >
                                       right->totalTicks;
                            });
                        const auto topScopeCount =
                            (std::min<std::size_t>)(
                                rankedScopeCount,
                                5);
                        ...
                        for (std::size_t rank = 0;
                            ...
                            const auto& item =
                                *rankedScopes[rank];
                            logger->info(
                                "[ROCK::Performance] Profiler TOP{} {}: aggregateMsPerFrame={:.4f} callsPerFrame={:.2f} totalMs={:.4f} maxCallMs={:.4f}",
                                rank + 1,
                                scopeName(item.scope),
                                item.totalMs() /
                                    measuredFrames,
                    ...
                    if (snapshot.benchmark.hasData()) {
                        const auto& benchmark = snapshot.benchmark;
                        logger->info(
                            "[ROCK::Performance] Benchmark mode={}({}) samples={} avgMs={:.3f} avgFPS={:.2f} p50Ms={:.3f} p90Ms={:.3f} p95Ms={:.3f} p99Ms={:.3f} maxMs={:.3f} onePercentLowFPS={:.2f}",
                            benchmark.mode,
                            benchmarkModeName(benchmark.mode),
                            benchmark.samples,
                            benchmark.averageMilliseconds,
                            benchmark.averageFps,
                            benchmark.p50Milliseconds,
                            benchmark.p90Milliseconds,
                            benchmark.p95Milliseconds,
                            benchmark.p99Milliseconds,
                            benchmark.maxMilliseconds,
                            benchmark.onePercentLowFps);
                    }

                    if (flushNoticeImmediately) {
                        // Notices occur at most twice per benchmark session.
                        // Flush them immediately so a missing/disabled DLL can
                        // be distinguished from an eligibility gate without
                        // waiting for a full measurement interval.
                        flushLogger();
                    } else {
                        flushIfDue();
                    }
        ...
        double benchmarkPercentileMilliseconds(
            std::uint64_t numerator,
            std::uint64_t denominator) noexcept
        {
            if (s_benchmarkAccum.samples == 0 || denominator == 0) {
                return 0.0;
            }

            // Nearest-rank percentile. Each regular bucket reports its upper
            // bound, which is conservative by at most 0.1 ms. The overflow
            // bucket reports the exact window maximum.
            const auto targetRank =
                (s_benchmarkAccum.samples * numerator + denominator - 1) /
                denominator;
            std::uint64_t cumulative = 0;
            for (std::size_t index = 0;
                 index < s_benchmarkAccum.histogram.size();
                 ++index) {
                cumulative += s_benchmarkAccum.histogram[index];
                if (cumulative < targetRank) {
                    continue;
                }

                if (index >= kBenchmarkHistogramRegularBins) {
                    return ticksToMilliseconds(s_benchmarkAccum.maxTicks);
                }
        ...
        BenchmarkSnapshot takeBenchmarkSnapshot(int mode) noexcept
        {
            BenchmarkSnapshot snapshot{};
            snapshot.mode = mode;
            snapshot.samples = s_benchmarkAccum.samples;
            if (snapshot.samples == 0) {
                return snapshot;
            }

            snapshot.averageMilliseconds =
                ticksToMilliseconds(s_benchmarkAccum.totalTicks) /
                static_cast<double>(snapshot.samples);
            snapshot.averageFps =
                snapshot.averageMilliseconds > 0.0 ?
                    1000.0 / snapshot.averageMilliseconds :
                    0.0;
            snapshot.p50Milliseconds =
                benchmarkPercentileMilliseconds(1, 2);
            snapshot.p90Milliseconds =
                benchmarkPercentileMilliseconds(9, 10);
            snapshot.p95Milliseconds =
                benchmarkPercentileMilliseconds(19, 20);
            snapshot.p99Milliseconds =
                benchmarkPercentileMilliseconds(99, 100);
            snapshot.maxMilliseconds =
                ticksToMilliseconds(s_benchmarkAccum.maxTicks);
            // "1% low" is the reciprocal of the p99 frame time. This avoids
            // a second sample buffer and keeps the game-thread sampler fixed
            // cost and allocation-free.
            snapshot.onePercentLowFps =
                snapshot.p99Milliseconds > 0.0 ?
                    1000.0 / snapshot.p99Milliseconds :
                    0.0;
            return snapshot;
        }

        void queueBenchmarkSnapshot(const BenchmarkSnapshot& snapshot) noexcept
        {
            QueuedSnapshot queued{};
            queued.benchmark = snapshot;
            asyncProfilerWriter().enqueue(std::move(queued));
        }

        void queueBenchmarkNotice(
            int mode,
            std::uint32_t intervalFrames,
            std::uint32_t warmupFrames,
            bool armed,
            bool firstEligible) noexcept
        {
            QueuedSnapshot queued{};
            queued.benchmarkNoticeMode = mode;
            queued.benchmarkNoticeIntervalFrames = intervalFrames;
            queued.benchmarkNoticeWarmupFrames = warmupFrames;
            queued.benchmarkArmedNotice = armed;
            queued.benchmarkFirstEligibleNotice = firstEligible;
            asyncProfilerWriter().enqueue(std::move(queued));
        }
    ...
    void refreshBenchmarkSettings(
        int mode,
        int logIntervalFrames,
        int warmupFrames) noexcept
    {
        const auto sanitizedMode = sanitizeBenchmarkMode(mode);
        const auto sanitizedInterval =
            sanitizeIntervalFrames(logIntervalFrames);
        const auto sanitizedWarmup =
            sanitizeWarmupFrames(warmupFrames);
        const auto previousMode =
            s_benchmarkSettings.mode.load(std::memory_order_acquire);
        const bool settingsChanged =
            s_benchmarkSettings.logIntervalFrames.load(
                std::memory_order_acquire) != sanitizedInterval ||
            s_benchmarkSettings.warmupFrames.load(
                std::memory_order_acquire) != sanitizedWarmup;

        // Publish off before mutating the game-thread accumulator. The host is
        // expected to call both APIs on the game thread, but this also makes a
        // stray concurrent observer fail closed during a hot reload.
        s_benchmarkSettings.mode.store(0, std::memory_order_release);
        s_benchmarkSettings.logIntervalFrames.store(
            sanitizedInterval,
            std::memory_order_release);
        s_benchmarkSettings.warmupFrames.store(
            sanitizedWarmup,
            std::memory_order_release);
        ...
        if (sanitizedMode == 0) {
            if (previousMode != 0 &&
                s_benchmarkAccum.samples > 0) {
                queueBenchmarkSnapshot(
                    takeBenchmarkSnapshot(previousMode));
            }
            if (previousMode != 0 || settingsChanged) {
                resetBenchmarkAccum(sanitizedWarmup);
            }
            return;
        }

        if (!ensureFrequencyReady() || !asyncProfilerWriter().start()) {
            resetBenchmarkAccum(sanitizedWarmup);
            return;
        }

        if (previousMode != sanitizedMode || settingsChanged) {
            if (previousMode != 0 &&
                s_benchmarkAccum.samples > 0) {
                // Preserve a useful partial window when the tester hot-switches
                // A/B modes or adjusts its interval. Previously those samples
                // were silently discarded unless they happened to land on the
                // exact configured interval.
                queueBenchmarkSnapshot(
                    takeBenchmarkSnapshot(previousMode));
            }
            resetBenchmarkAccum(sanitizedWarmup);
            queueBenchmarkNotice(
                sanitizedMode,
                sanitizedInterval,
                sanitizedWarmup,
                true,
        ...
        s_benchmarkSettings.mode.store(
            sanitizedMode,
            std::memory_order_release);
    }
    ...
    void observeFrameBoundary(bool eligible) noexcept
    {
        ...
        if (!eligible) {
            s_benchmarkAccum.previousBoundaryTicks = 0;
            return;
        }

        if (!s_benchmarkAccum.firstEligibleNoticeQueued) {
            s_benchmarkAccum.firstEligibleNoticeQueued = true;
            queueBenchmarkNotice(
                mode,
                s_benchmarkSettings.logIntervalFrames.load(
        ...
        const auto now = queryPerformanceTicks();
        if (now == 0) {
            s_benchmarkAccum.previousBoundaryTicks = 0;
            return;
        }

        const auto previous = s_benchmarkAccum.previousBoundaryTicks;
        s_benchmarkAccum.previousBoundaryTicks = now;
        if (previous == 0 || now <= previous) {
            return;
        }

        const auto elapsedTicks = now - previous;
        if (s_benchmarkAccum.warmupRemaining > 0) {
            --s_benchmarkAccum.warmupRemaining;
            if (s_benchmarkAccum.warmupRemaining == 0) {
                clearBenchmarkMeasurement();
            }
            return;
        }
        ...
        const auto bucket = std::min<std::size_t>(
            static_cast<std::size_t>(
                elapsedMicroseconds /
                kBenchmarkHistogramBinMicroseconds),
            kBenchmarkHistogramRegularBins);
        ++s_benchmarkAccum.histogram[bucket];
        s_benchmarkAccum.totalTicks += elapsedTicks;
        s_benchmarkAccum.maxTicks =
            std::max(s_benchmarkAccum.maxTicks, elapsedTicks);
        ++s_benchmarkAccum.samples;

        const auto interval =
            s_benchmarkSettings.logIntervalFrames.load(
                std::memory_order_acquire);
        if (s_benchmarkAccum.samples < interval) {
            return;
        }

        const auto snapshot = takeBenchmarkSnapshot(mode);
        clearBenchmarkMeasurement();
        queueBenchmarkSnapshot(snapshot);
    }

    int benchmarkMode() noexcept
    {
        return s_benchmarkSettings.mode.load(std::memory_order_acquire);
    }
