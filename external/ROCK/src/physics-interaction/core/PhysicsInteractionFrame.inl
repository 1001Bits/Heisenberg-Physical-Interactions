PhysicsFrameContext PhysicsInteraction::buildFrameContext(RE::bhkWorld* bhk, RE::hknpWorld* hknp, float deltaSeconds)
{
    ...
    /*
     * Heisenberg-preserved grab-locomotion authority bridge (upstream removed the
     * subsystem in 6452acd). Runs before any hand input is built so every consumer
     * of rawHandWorld this frame sees the same lead-compensated pose.
     */
