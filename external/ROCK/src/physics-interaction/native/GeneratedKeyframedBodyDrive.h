...
namespace rock
{
    namespace generated_keyframed_body_drive_math
    {
        ...
        // SNAP-TELEPORT SPEED CEILING (Jul 31). A per-sample target jump faster
        // than any humanly-plausible source motion is a pose DISCONTINUITY
        // (dialogue-pose snap-back, skeleton hand-off, menu transition), not
        // movement — sweeping a keyframed collider through it shoves whatever
        // lies along the arc (live: a beer bottle knocked off a table BEHIND
        // the player at a DialogueMenu close). 2500 gu/s ≈ 35 m/s keeps >=2x
        // headroom over the fastest measured legitimate motion (a bat-tip hull
        // segment at ~16 m/s) at every frame rate, because the threshold
        // scales with the source delta. The floor keeps very short deltas from
        // producing hair-trigger thresholds. A tripped teleport is benign:
        // placeGeneratedKeyframedBodyImmediately zeroes velocity and rebuilds
        // pair caches, so the body arrives without pushing anything.
        inline constexpr float kMaxPlausibleSourceSpeedGameUnitsPerSecond = 2500.0f;
        inline constexpr float kMinTeleportDistanceGameUnits = 25.0f;
