...
namespace rock
{
    ...
    public:
        ...
        void onBeforeWholePhysicsUpdate(RE::hknpWorld* registeredWorld, std::uint64_t registrationSequence);
        void onBeforeAnyPhysicsStep(
            RE::hknpWorld* registeredWorld,
            std::uint64_t registrationSequence,
            float substepProgress,
            float substepDeltaSeconds);
        void onBetweenCollideAndSolve(
            RE::hknpWorld* registeredWorld,
            std::uint64_t registrationSequence,
            float substepProgress,
            float substepDeltaSeconds);
        void onAfterAnyPhysicsStep(
            RE::hknpWorld* registeredWorld,
            std::uint64_t registrationSequence,
            float substepProgress,
            float substepDeltaSeconds);
    ...
    private:
        ...
        [[nodiscard]] bool acceptsRegistration(
            RE::hknpWorld* registeredWorld,
        ...
        /*
         * Process-lifetime because bhkWorld can retain an old listener after
         * this coordinator is reset/destroyed. All listener generations share
         * this gate, while each owns its own whole-update lease.
         */
        PhysicsCallbackQuiescenceGate* _callbackGate = nullptr;
