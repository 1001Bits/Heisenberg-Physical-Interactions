...
// Host-side, SEH-guarded, idempotent clear of the player char-proxy's cached support body.
// Declared here rather than including src/Physics.h so the engine keeps a single narrow seam
// into the host (same arrangement shipped in 0.8.4).
namespace heisenberg::Physics { void ClearPlayerProxySupportBody(); }
...
namespace rock
{
    ...
    void BethesdaPhysicsBody::destroy(void* bhkWorld)
    {
        ...
        // CRASH FIX (Jul 5; restored after the attempt-6 port dropped it): clear the player
        // character-proxy's cached support/contact body BEFORE we free this collider body. The
        // char-controller's per-step constraint callback (hookedProcessConstraintsCallback /
        // bhkCharProxyController::processConstraints) caches and dereferences contact bodies; if it
        // cached THIS body and we free it, the next native solve reads freed memory -> CTD (the
        // classic char-proxy dangling-body crash). This became reliably fatal once the keyframed
        // hand capsules are kept awake every frame (they then generate char-proxy contacts) and get
        // torn down/rebuilt ~2x/sec during a two-handed grip. The host's clear is SEH-guarded and
        // idempotent. Only when a real world body is being removed.
        if (bhkWorld && (_created || _collisionObject)) {
    ...
    namespace
    {
        ...
        std::atomic<bool> s_hasRetiredDeferredBodies{ false };
    }
    ...
    void BethesdaPhysicsBody::retireDeferred(void* bhkWorld)
    {
        ...
        for (auto& retired : s_retiredDeferredBodies) {
            if (!retired.occupied()) {
                ...
                s_hasRetiredDeferredBodies.store(
                    true,
    ...
    void BethesdaPhysicsBody::serviceRetiredDeferredPayloads(std::uint32_t completedPhysicsSteps)
    {
        ...
        if (!s_hasRetiredDeferredBodies.load(
