...
namespace rock
{
    namespace
    {
        ...
        std::uint64_t bodyColliderTuningSignature(bool inPowerArmor)
        {
            ...
            // Arm-segment padding shapes these capsules too (host-driven MCM slider) -
            // include it so a mid-session change forces the same rebuild the scales get.
            mixBodyColliderSignature(signature, g_rockConfig.rockHandBoneColliderRadiusPadding);
            return signature;
        }
            ...
            BodyBoneColliderSet::DescriptorFrameResult& outFrame)
        {
            ...
            // HOST DIVERGENCE (Heisenberg embed): the host-driven hand/arm radius padding
            // (MCM slider, bridged via HostSetHandColliderRadiusPadding) also covers the ARM
            // segments - the rendered forearm mesh extends past the stock capsules just like
            // the fingers do, and the user-reported clip-through (duct tape resting on the
            // forearm) lives here, not in the hand set. Deliberately NOT applied to torso/
            // leg/foot capsules: those interact with the character-controller ground probe
            // and are exactly where padding could destabilize locomotion.
            switch (descriptor.role) {
            case BoneColliderRole::UpperArmSegment:
            case BoneColliderRole::ForearmSegment:
            case BoneColliderRole::HandSegment:
            case BoneColliderRole::FingerSegment:
                input.radius += std::clamp(g_rockConfig.rockHandBoneColliderRadiusPadding, 0.0f, 3.0f);
                break;
            default:
                break;
            }
    ...
    const DirectSkeletonBoneSnapshot* BodyBoneColliderSet::captureBoneSnapshot()
    {
        ...
        BodyInstance& instance)
    {
        ...
        if (!placeGeneratedKeyframedBodyImmediately(world, instance.body, frame.transform)) {
            ROCK_LOG_ERROR(Body,
    ...
    bool BodyBoneColliderSet::create(RE::hknpWorld* world, void* bhkWorld)
    {
        ...
        const auto* sharedSnapshot = captureBoneSnapshot();
        if (!sharedSnapshot) {
