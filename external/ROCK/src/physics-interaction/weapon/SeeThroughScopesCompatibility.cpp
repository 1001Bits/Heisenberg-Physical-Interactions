...
namespace
{
        ...
        const RE::TBO_InstanceData* /*instanceData*/)
    {
         ...
         * equipped weapon's BGSObjectInstanceExtra (used here to detect an attached
         * scope mod). Until the b46b8dd inventory walk is implemented, scope-route
         * detection via attachment indices is unavailable and see-through-scopes
         * compatibility stays inert (the subsystem already no-ops without it).
         */
        return nullptr;
    }
    ...
    void alignReticle(const EquippedScopeRouteSnapshot& equippedScope)
    {
        ...
        auto* cameraNode = camera && camera->cameraRoot ? camera->cameraRoot.get() : nullptr;
...
namespace rock::see_through_scopes
{
    bool installLateCullingHook()
    {
        ...
        ROCK_LOG_INFO(Scope, "Hooking late culling at 0x{:X} ({}).", hookCallSite.address(), rock::hook_diagnostics::describeAddress(hookCallSite.address()));

        /*
         * CONSEQUENCE OF RETURNING FALSE, stated here once because the failure messages
         * below have to carry it: ROCKMain.cpp's engine-load path calls this function
         * inside `if (GetModuleHandleA("FO4VR_better_scopes.dll")) { if (!installLateCullingHook())
         * return false; }`, and that `return false` aborts the ENTIRE embedded ROCK engine
         * load. A failure here is therefore NOT "scope compatibility turns off" - it is
         * "ROCK does not load at all". The previous message said the former, which
         * understated the damage by an entire feature set.
         */

        // Validate the call-site prefix before patching (matches the project rule the
        // neighbouring grenade-equip hook already follows - ROCKMain.cpp's
        // installEquipHook fails closed the same way). write_call<5> decodes the
        // "original" target from whatever bytes are AT this address without checking
        // they're really a 0xE8 call first; if this site has already been detoured,
        // write_call<5> would decode a displacement out of the foreign patch bytes into
        // a garbage non-null s_originalLateCulling, and the first late-culling pass
        // would call through it.
        const auto* siteBytes = reinterpret_cast<const std::uint8_t*>(hookCallSite.address());
        if (siteBytes[0] != 0xE8) {
            // Findings only: address, owning module, the bytes present, and - if those
            // bytes are a branch - where it goes. The old "another mod likely hooks this
            // site" clause was a hardcoded guess; nothing here probes for another mod.
            std::string branchNote = rock::hook_diagnostics::describeBranchAtAddress(hookCallSite.address());
            if (!branchNote.empty()) {
                branchNote.insert(0, " ");
            }
            ROCK_LOG_CRITICAL(Scope,
                "ROCK ENGINE LOAD ABORTED: the See-Through-Scopes late-culling call site did not validate. "
                "Site 0x{:X} ({}): expected a CALL rel32 (opcode 0xE8), found bytes [{}].{} "
                "CONSEQUENCE: this is not merely scope compatibility switching off - ROCKMain aborts the whole embedded ROCK engine load on this "
                "failure, so with FO4VR_better_scopes.dll present you get NO hand or weapon collision, NO physical grabbing and NO held-object "
                "physics this session.",
        ...
        if (!s_originalLateCulling) {
            ROCK_LOG_CRITICAL(Scope,
                "ROCK ENGINE LOAD ABORTED: F4SE trampoline write_call<5> at 0x{:X} ({}) returned a null original-function pointer. "
                "CONSEQUENCE: ROCKMain aborts the whole embedded ROCK engine load on this failure - no hand or weapon collision, no physical "
                "grabbing and no held-object physics this session.",
                hookCallSite.address(),
                rock::hook_diagnostics::describeAddress(hookCallSite.address()));
            return false;
        }

        ROCK_LOG_INFO(Scope, "Late culling hook installed, original: 0x{:X} ({}).", original, rock::hook_diagnostics::describeAddress(original));
        return true;
    }
