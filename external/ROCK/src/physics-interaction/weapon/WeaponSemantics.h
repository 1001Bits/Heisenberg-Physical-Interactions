...
/*
 * Weapon semantic classification helpers are grouped here because generated geometry still needs part metadata for contact, reload, and debug evidence.
 */
...
namespace rock
{
    ...
    inline WeaponPartClassification classifyWeaponPartName(std::string_view sourceName)
    {
        ...
        /*
         * A release control is not the moving bolt/slide that it releases.
         * Check these tokens before the broad "bolt"/"slide" matches below:
         * Fallout 4's stock 10 mm names the control
         * "Pistol10mmBoltRelease:0", which otherwise produces a second Bolt
         * evidence record and makes MatchPartKind ambiguous.
         */
        if (weaponPartNameContains(sourceName, "boltrelease") ||
            weaponPartNameContains(sourceName, "bolt release") ||
            weaponPartNameContains(sourceName, "bolt_release") ||
            weaponPartNameContains(sourceName, "sliderelease") ||
            weaponPartNameContains(sourceName, "slide release") ||
            weaponPartNameContains(sourceName, "slide_release")) {
