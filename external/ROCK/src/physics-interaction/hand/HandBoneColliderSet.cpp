...
namespace rock
{
    namespace
    {
        ...
        float roleRadius(HandColliderRole role, bool powerArmor)
        {
            ...
            case HandFingerSegment::Base:
                return 0.55f * scale + padding;
            case HandFingerSegment::Middle:
                return 0.46f * scale + padding;
            case HandFingerSegment::Tip:
                return 0.38f * 1.10f * scale + padding;
            default:
                return 0.5f * scale + padding;
            }
        }
        ...
        std::uint64_t handColliderTuningSignature(bool powerArmor)
        {
            ...
            std::memcpy(&paddingBits, &g_rockConfig.rockHandBoneColliderRadiusPadding, sizeof(paddingBits));
            signature ^= static_cast<std::uint64_t>(paddingBits) + 0x9E37'79B9'7F4A'7C15ull + (signature << 6) + (signature >> 2);
            return signature;
        }
    ...
    std::vector<RE::NiPoint3> HandBoneColliderSet::makeLocalCollisionPointsForRole(
        ...
        const RE::NiPoint3& authorityTranslationOffsetGame)
    {
        ...
        if (!placeGeneratedKeyframedBodyImmediately(world, palmAnchorBody, anchorFrame.transform)) {
            ROCK_LOG_ERROR(Hand,
