...
namespace rock
{
    ...
    void Hand::clearHeldBodyIdsSnapshotPublication()
    {
        _heldBodyIdsSnapshotSequence.fetch_add(1, std::memory_order_acq_rel);
        _heldBodyIdsCount.store(0, std::memory_order_release);
        _heldBodyIdsSnapshotSequence.fetch_add(1, std::memory_order_release);
    }

    void Hand::publishHeldBodyIdsSnapshotPublication()
    {
        _heldBodyIdsSnapshotSequence.fetch_add(1, std::memory_order_acq_rel);
        _heldBodyIdsCount.store(0, std::memory_order_release);
        const int count = static_cast<int>((std::min)(
            _heldBodyIds.size(),
            static_cast<std::size_t>(MAX_HELD_BODIES)));
        for (int index = 0; index < count; ++index) {
            _heldBodyIdsSnapshot[static_cast<std::size_t>(index)].store(
                _heldBodyIds[static_cast<std::size_t>(index)],
    ...
    void Hand::abandonHavokStateAfterWorldLoss()
    {
        ...
        havok_runtime::abandonBodyLeasesForOwner(
