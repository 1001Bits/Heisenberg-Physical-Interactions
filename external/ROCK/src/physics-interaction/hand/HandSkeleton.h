...
namespace rock
{
    ...
    {
        /*
         * Frame-scoped view into DirectSkeletonBoneReader's stable tree cache.
         * A snapshot must not outlive its reader's next cache rebuild/reset;
         * the shared runtime API enforces the same updateFrame generation.
         */
...
namespace rock
{
    ...
    public:
         ...
         */
        bool resolve(const DirectSkeletonBoneSnapshot* snapshot)
        {
            ...
            if (!findBone(*snapshot, "RArm_Hand", rightHand) ||
                !findBone(*snapshot, "LArm_Hand", leftHand)) {
                clearResolvedState();
                return false;
            }
