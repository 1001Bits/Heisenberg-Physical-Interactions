...
namespace rock
{
    ...
    {
        // Keep the frame-shared snapshot's vector storage alive. Before the
        // shared runtime path this object was temporary at every call, so the
        // aggregate reset's deallocation was hidden among several larger
        // per-consumer copies.
        outSnapshot.valid = false;
        outSnapshot.inPowerArmor = false;
        outSnapshot.mode = DebugSkeletonBoneMode::Off;
        outSnapshot.source = SkeletonBoneSnapshotSource::None;
        outSnapshot.skeleton = nullptr;
        outSnapshot.boneTree = nullptr;
        outSnapshot.totalBoneCount = 0;
        outSnapshot.requiredResolvedCount = 0;
        outSnapshot.bones.clear();
        outSnapshot.missingRequiredBones.clear();
    ...
    bool DirectSkeletonBoneReader::captureFromCachedTree(DirectSkeletonBoneSnapshot& outSnapshot)
    {
        ...
        for (const auto& cached : _cachedBones) {
            ...
            outSnapshot.bones.push_back(DirectSkeletonBoneEntry{
                .name = std::string_view{ cached.name },
