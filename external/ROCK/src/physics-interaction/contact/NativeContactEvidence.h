...
namespace rock::contact_evidence
{
    ...
    inline constexpr bool sameContactIdentity(
        const NativeContactEvidenceRecord& lhs,
        const NativeContactEvidenceRecord& rhs)
    {
        return lhs.sourceBodyId == rhs.sourceBodyId &&
               lhs.targetBodyId == rhs.targetBodyId &&
    ...
    public:
        ...
        void snapshot(NativeContactEvidenceSnapshot& outSnapshot, std::uint32_t currentFrame) const
        {
            ...
            for (const auto& record : _records) {
                ...
                NativeContactEvidenceRecord* existing = nullptr;
                for (std::uint32_t index = 0;
                     index < outSnapshot.count;
                     ++index) {
                    if (sameContactIdentity(
                            outSnapshot.records[index],
                            record)) {
                        existing = &outSnapshot.records[index];
                        break;
                    }
                }
                ...
                if (outSnapshot.count < outSnapshot.records.size()) {
