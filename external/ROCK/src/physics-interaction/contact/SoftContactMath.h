...
namespace rock::soft_contact_math
{
    ...
    /*
     * Controller-relative soft-contact tags bypass the host's ordinary
     * free-hand reach clamp. Project the final published target here so a wall
     * can stop the palm without asking FRIK to solve an anatomically impossible
     * shoulder-to-hand distance when locomotion carries the body onward.
     */
    inline ArmReachProjection projectHandTargetToArmReach(
        const RE::NiPoint3& target,
        const RE::NiPoint3& shoulder,
        float maxReach,
        float safetyMargin = 0.15f)
    {
        ArmReachProjection result{};
        result.target = target;
        if (!isFinite(target) || !isFinite(shoulder) ||
            !std::isfinite(maxReach) || maxReach <= 1.0f ||
            !std::isfinite(safetyMargin) || safetyMargin < 0.0f) {
            return result;
        }

        const RE::NiPoint3 shoulderToTarget = sub(target, shoulder);
        const float distanceSquared = lengthSquared(shoulderToTarget);
        if (!std::isfinite(distanceSquared) || distanceSquared < 0.0f) {
            return result;
        }

        const float requestedDistance = std::sqrt(distanceSquared);
        const float reachableDistance = (std::max)(maxReach - safetyMargin, 0.0f);
        result.requestedDistance = requestedDistance;
        result.appliedDistance = requestedDistance;
        result.valid = true;
        if (requestedDistance <= reachableDistance || requestedDistance <= 1.0e-6f) {
            return result;
        }

        result.target = add(
            shoulder,
            mul(shoulderToTarget, reachableDistance / requestedDistance));
        result.appliedDistance = reachableDistance;
