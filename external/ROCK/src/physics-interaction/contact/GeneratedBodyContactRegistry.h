...
namespace rock::generated_body_contact_registry
{
    ...
    public:
        ...
        void publish(const Entry* entries, std::size_t count)
        {
            ...
            // Readers search only [0, _count), and the version bracket makes a
            // reader retry if it overlaps this publication. Slots above the new
            // count are therefore unreachable; zeroing all of them every frame
            // was up to thousands of needless release stores for the 163-slot
            // hand + weapon + body registry.
