...
#define ROCK_CALL __cdecl
    ...
    // NOTE (Heisenberg vendored copy): bumped to 5 to describe the appended
    // forceGrab/canForceGrab members (see ROCK_forceGrab_API_Request.md). ROCK
    // releases <5 simply lack those trailing members; RockBridge binds with an
    // explicit minVersion of 4 and only ever reads forceGrab when the live
    // ROCK reports getVersion() >= 5, so this stays ABI-safe against older ROCK.
    inline constexpr std::uint32_t ROCK_API_VERSION = 5;
    ...
    struct ROCKApi
    {
        ...
        // ---- v5 additions (Heisenberg request; see ROCK_forceGrab_API_Request.md) ----
        // Begin holding `refr` in `hand` as if the player grip-grabbed it from the
        // world (instant, no pull). Returns true if committed. Only valid when the
        // live ROCK reports getVersion() >= 5. Appended last to preserve ABI.
        bool(ROCK_CALL* forceGrab)(Hand hand, RE::TESObjectREFR* refr);
        // True if forceGrab(hand, ...) could currently succeed (hand free, physics
        // hand enabled, world/skeleton ready, not menu/loading). Optional helper.
        bool(ROCK_CALL* canForceGrab)(Hand hand);
