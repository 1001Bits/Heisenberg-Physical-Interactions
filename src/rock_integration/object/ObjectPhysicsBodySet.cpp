// 1:1 port of ROCK physics-interaction/object/ObjectPhysicsBodySet.cpp (replaces the earlier
// partial #109 adaptation, which left motionId/motionType/bodyFlags unpopulated — making the
// classifier reject every body — and dropped seeding, scan-policy options, multi-body
// enumeration, and the surface-owner ancestor matching).
//
// Documented adaptations only:
//  - havok_runtime::getBody/getMotion -> the grab-core shim (BodyView exposes the Ghidra-verified
//    hknpBody fields: position@0x30, flags@0x40, collisionFilterInfo@0x44, motionIndex@0x68).
//  - resolveBodyToRef -> heisenberg::Physics::GetRefrFromBodyId.
//  - forEachPhysicsSystemBodyIdDetailed (missing from the vendored ROCK source) -> an SEH-leaf
//    index loop over the proven BhkPhysicsSystemGetBodyId(sys, &id, idx) engine call, capped at
//    256 bodies like ROCK, with an equivalent status model.
//  - body->motionPropertiesId fallback (used by ROCK only when the motion is null) reads 0 —
//    the hknpBody copy of that field has no verified Heisenberg offset; flags-derived motion
//    type dominates and a null motion is already the failure edge.
//  - ROCK_LOG -> spdlog (level-gated).
...
namespace heisenberg::rock_core::object_physics_body_set
{
    namespace
    {
        ...
        bool isWeaponReference(RE::TESObjectREFR* ref)
        {
            ...
            return baseForm && baseForm->GetFormType() == RE::ENUM_FORM_ID::kWEAP;
        }

        // ---- physics-system body enumeration (adapter for ROCK's missing-source
        //      havok_runtime::forEachPhysicsSystemBodyIdDetailed) -------------------------------
        ...
        {
            PhysicsSystemBodyScanStatus status = PhysicsSystemBodyScanStatus::NoCollisionObject;
            int bodyCount = 0;
            [[nodiscard]] bool enumerated() const { return status == PhysicsSystemBodyScanStatus::Enumerated; }
        };

        inline constexpr int kMaxPhysicsSystemBodies = 256;  // ROCK's enumeration cap

        // SEH leaf (PODs only): walk the bhkPhysicsSystem body table by index via the proven
        // engine getter. Stops at the first INVALID id (end of table) or the cap. Returns the
        // count collected, or -1 on fault (unreadable system).
        static int SafeCollectSystemBodyIds(void* sys, std::uint32_t* out, int cap)
        {
            int n = 0;
            __try {
                for (int idx = 0; idx < cap; ++idx) {
                    std::uint32_t bodyId = INVALID_BODY_ID;
                    heisenberg::ConstraintFunctions::BhkPhysicsSystemGetBodyId(sys, &bodyId, static_cast<std::uint32_t>(idx));
        ...
        // 1:1 with ROCK's appendBodyRecord.
        bool appendBodyRecord(ScanBodyContext& context, std::uint32_t rawBodyId, bool seeded)
        {
            ...
            auto* body = heisenberg::rock_core::havok_runtime::getBody(context.hknpWorld, RE::hknpBodyId{ rawBodyId });
            if (!body) {
                return true;
            }
            ...
            void* motion = heisenberg::rock_core::havok_runtime::getMotion(context.hknpWorld, record.motionId);
            const std::uint16_t motionPropertiesId = heisenberg::rock_core::havok_runtime::motionPropertiesIdOf(motion);
            ...
            record.positionGame.x = body->position[0] * havokToGameScale();
            record.positionGame.y = body->position[1] * havokToGameScale();
            record.positionGame.z = body->position[2] * havokToGameScale();

            auto* resolved = heisenberg::Physics::GetRefrFromBodyId(context.bhkWorld, rawBodyId);
            if (resolved) {
            ...
            if (!record.accepted) {
                ...
                spdlog::debug("[ROCK::ObjectBodySet] BODY reject: body={} reason={} layer={} motionId={} node='{}'",
                              rawBodyId,
        ...
        // 1:1 with ROCK's scanNode (enumeration adapter substituted, see header comment).
            ...
            std::unordered_set<std::uint32_t>& seenBodyIds)
        {
            ...
            if (collisionObject) {
                ...
                PhysicsSystemBodyScanResult scanResult{};
                auto* npco = static_cast<RE::bhkNPCollisionObject*>(collisionObject);
                void* sys = npco->spSystem.get();
                if (!sys) {
                    scanResult.status = PhysicsSystemBodyScanStatus::NoPhysicsSystem;
                } else {
                    std::uint32_t bodyIds[kMaxPhysicsSystemBodies];
                    const int n = SafeCollectSystemBodyIds(sys, bodyIds, kMaxPhysicsSystemBodies);
                    if (n < 0) {
                        scanResult.status = PhysicsSystemBodyScanStatus::UnreadableBodyIds;
                    } else {
                        scanResult.status = PhysicsSystemBodyScanStatus::Enumerated;
                        scanResult.bodyCount = n;
                        for (int i = 0; i < n; ++i) {
                            if (!appendBodyRecord(context, bodyIds[i], false)) {
                                break;
                            }
                        }
                    }
                ...
                if (!scanResult.enumerated()) {
                    if (scanResult.status == PhysicsSystemBodyScanStatus::UnreadableBodyIds) {
                        ++out.diagnostics.scanFailures;
                        ++out.diagnostics.invalidPhysicsSystems;
                        spdlog::warn("[ROCK::ObjectBodySet] scan skipped invalid physics system: rootFormID={:08X} node='{}'",
                                     rootRef ? rootRef->GetFormID() : 0, nodeName(node));
                    } else {
            ...
            for (std::uint32_t i = 0; i < niNode->children.size(); ++i) {
    ...
    // ---- ObjectPhysicsBodySet accessors (1:1 with ROCK) ----
    ...
    ObjectPhysicsBodySet scanObjectPhysicsBodySet(RE::bhkWorld* bhkWorld, RE::hknpWorld* hknpWorld, RE::TESObjectREFR* ref, const BodySetScanOptions& options)
    {
        ...
        if (options.seedBodyId != INVALID_BODY_ID) {
            ScanBodyContext seedContext{ bhkWorld, reinterpret_cast<void*>(hknpWorld), ref, options.seedHitNode ? options.seedHitNode : result.rootNode, nullptr, &options, &result, &seenBodyIds };
            appendBodyRecord(seedContext, options.seedBodyId, true);
        }

        if (options.allowWeaponRefExpansion || !isWeaponReference(ref)) {
            scanNode(bhkWorld, reinterpret_cast<void*>(hknpWorld), ref, result.rootNode, (std::max)(0, options.maxDepth), options, result, seenBodyIds);
        } else {
            ++result.diagnostics.weaponExpansionSkips;
        }

        spdlog::debug("[ROCK::ObjectBodySet] scanned {:08X}: {} bodies ({} accepted, {} rejected)",
