...
// 1:1 port of ROCK physics-interaction/native/HavokTimingFixPolicy.h — pure math that
// decides the physics substep delta + count to enforce a minimum physics frame rate
// (more substeps when the render frame is long), clamped to the engine's 6-substep max.
// Namespace kept as heisenberg::rock_core::havok_timing_fix_policy (header-only). Driven by the call-site
// hook on bhkWorld::SetDeltaTime (see HavokTimingFix.cpp).

#include <algorithm>
#include <cmath>
#include <cstdint>

namespace heisenberg::rock_core::havok_timing_fix_policy
{
    ...
    inline TimingFixDecision evaluateTimingFix(const TimingFixInput& input)
    {
        ...
        if (!isUsableDeltaSeconds(input.rawDeltaSeconds)) {
            return TimingFixDecision{ .valid = false, .maxPhysicsFrameSeconds = maxPhysicsFrameSeconds, .clampedMaxSubsteps = maxSubsteps, .reason = "invalidRawDelta" };
        }
        if (!isUsableDeltaSeconds(input.accumulatedDeltaSeconds)) {
            return TimingFixDecision{ .valid = false, .maxPhysicsFrameSeconds = maxPhysicsFrameSeconds, .clampedMaxSubsteps = maxSubsteps, .reason = "invalidAccumulatedDelta" };
        }
        ...
        if (!isUsableDeltaSeconds(substepDeltaSeconds)) {
            return TimingFixDecision{ .valid = false, .maxPhysicsFrameSeconds = maxPhysicsFrameSeconds, .clampedMaxSubsteps = maxSubsteps, .reason = "invalidSubstepDelta" };
        }
