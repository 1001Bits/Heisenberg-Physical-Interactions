...
// 1:1 port of ROCK physics-interaction/collision/PushAssist.h — the impulse-based
// hand→object push model. Pure, templated math (no engine deps), namespaced under
// heisenberg::rock_push_assist so it can coexist with the real ROCK source.
//
// This is the alternative to Heisenberg's velocity-replace push (HandCollision::
// ApplyPushForce / SetLinearVelocity): ROCK computes a push IMPULSE along the hand's
// velocity, clamped by maxImpulse, scaled by a per-layer multiplier, gated by a min
// speed + cooldown, then applies it as a velocity delta (accumulates, preserving the
// object's existing motion) rather than overwriting velocity.
//
// Math is verbatim from ROCK; only the namespace differs. Runtime apply
// (applyLinearImpulse / deferred velocity delta) is intentionally NOT ported here —
// Heisenberg drives the result through its own body-velocity path when wired.

#include <algorithm>
#include <cmath>
#include <cstdint>

namespace heisenberg::rock_push_assist
{
