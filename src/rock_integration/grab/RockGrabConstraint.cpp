...
#include "rock_integration/grab/HavokOffsets.h"
...
namespace heisenberg::rock_core
{
    ...
    // [GC-DIAG Jul 18] latest runtime block the solver registered for OUR constraint data.
    // If its solver-results words stay all-zero across post-commit frames, the solver never
    // touched the constraint (created-but-not-solved); nonzero-but-object-falls = wrong target.
    static void* g_lastConstraintRuntime = nullptr;
    static int g_lastConstraintRuntimeSize = 0;
    ...
    static void __cdecl addInstanceCallback(void* thisPtr, void* constraint, void* runtime, int sizeOfRuntime)
    {
        ...
        g_lastConstraintRuntime = runtime;
        g_lastConstraintRuntimeSize = sizeOfRuntime;
        if (runtime) {
    ...
    // SEH-safe native constraint create (review F7 — mirrors HeldBody's SafeCreateConstraint).
    // Plain function, no C++ objects inside __try.
    static bool safeCreateConstraintNative(void* world, heisenberg::hknpConstraintId* outId,
                                           heisenberg::hknpConstraintCinfo* cinfo)
    {
        __try {
            heisenberg::ConstraintFunctions::CreateConstraint(world, outId, cinfo);
            return true;
    ...
    ActiveConstraint createGrabConstraint(void* world, RE::hknpBodyId handBodyId, RE::hknpBodyId objectBodyId,
        ...
        *reinterpret_cast<std::uint16_t*>(header + ATOM_LIN_MOTOR_2) = ATOM_TYPE_LIN_MOTOR;
...

        {
            ...
            const RE::NiPoint3 pivotAProxyLocalGame =
                grab_constraint_math::computeGeneratedProxyConstraintPivotLocalGame(handBodyWorld, palmWorldGame);
            ...
            tA_pos[0] = pivotAProxyLocalGame.x * gameToHkScale;
            tA_pos[1] = pivotAProxyLocalGame.y * gameToHkScale;
            tA_pos[2] = pivotAProxyLocalGame.z * gameToHkScale;
            tA_pos[3] = 0.0f;
        ...
        // Heisenberg uses the raw hknp CreateConstraint free function + an explicit body-map
        // register (ROCK called the bhk-level world->CreateConstraint method that folds the
        // body-map in). Mirror HeldBodyGrab's proven sequence: CreateConstraint -> AddConstraintBodyMap.
        heisenberg::hknpConstraintCinfo cinfo{};
        cinfo.constraintData = cd;
        cinfo.bodyIdA = handBodyId.value;
        cinfo.bodyIdB = objectBodyId.value;
        // wantRuntime (Jul 5, P4): forces hknp to allocate the constraint runtime buffer the
        // motors solve into. The legacy proven paths always set this (GrabConstraint.cpp:1014/
        // 1132, HeldBodyGrab.cpp:2769 "forces hknp to allocate constraint runtime buffer");
        // without it the position/ragdoll motors can be dead (no runtime to write results).
        cinfo.flags = 1;

        heisenberg::hknpConstraintId outIdObj{};
        // SEH-wrapped (review F7): the proven HeldBody twin wraps this exact native call
        // (SafeCreateConstraint) — a bad body id / stale world faults inside CreateConstraint.
        std::uint32_t outId = 0x7FFF'FFFF;
        if (safeCreateConstraintNative(world, &outIdObj, &cinfo)) {
            outId = outIdObj.m_value;
        }
        if (outId != 0x7FFF'FFFF) {
            heisenberg::ConstraintFunctions::AddConstraintBodyMap(world, outIdObj, &cinfo);
        }
        ...
        const RE::NiTransform& desiredBodyTransformHandSpace, float tau, float damping, float maxForce, float proportionalRecovery, float constantRecovery)
    {
        ...
        return createGrabConstraint(
            ...
            GrabConstraintMotorTuning{
                ...
                .angularTau = g_grabCoreConfig.rockGrabAngularTau,
                .angularDamping = g_grabCoreConfig.rockGrabAngularDamping,
                .angularProportionalRecovery = g_grabCoreConfig.rockGrabAngularProportionalRecovery,
                .angularConstantRecovery = g_grabCoreConfig.rockGrabAngularConstantRecovery,
                .angularMaxForce = linearMaxForce,
                .ragdollDecompositionConfigMode =
                    grab_constraint_math::sanitizeGrabRagdollDecompositionConfigMode(g_grabCoreConfig.rockGrabRagdollDecompositionMode),
            });
    }

    void destroyGrabConstraint(void* world, ActiveConstraint& constraint)
    {
        ...
        if (world) {
            // Mirror Heisenberg teardown: unregister body-map (world, 0, id) THEN DestroyConstraints.
            heisenberg::ConstraintFunctions::RemoveConstraintBodyMap(world, 0, constraint.constraintId);
            heisenberg::hknpConstraintId ids[1] = { { constraint.constraintId } };
            heisenberg::ConstraintFunctions::DestroyConstraints(world, ids, 1);
    ...
    namespace
    {
        bool normalizeGrabbedInertiaMotion(void* world, RE::hknpBodyId bodyId, SavedObjectState::SavedMotionInertiaState& savedMotionState)
        {
            if (!world)
                return false;
            ...
            const auto normalizedInertia = grab_inertia_policy::normalizeInverseInertiaAxesForGrab(
                ...
                invI[1],
                invI[2],
                g_grabCoreConfig.rockGrabMaxInertiaRatio,
                g_grabCoreConfig.rockGrabMinInertia);
    ...
    void normalizeGrabbedInertia(void* world, RE::hknpBodyId bodyId, SavedObjectState& savedState)
    {
    ...
    void normalizeGrabbedInertiaForBodies(void* world, RE::hknpBodyId primaryBodyId, const std::vector<std::uint32_t>& heldBodyIds, SavedObjectState& savedState)
    {
        savedState.motionInertiaStates.clear();
        savedState.inertiaModified = false;
    ...
    void restoreGrabbedInertia(void* world, SavedObjectState& savedState)
    {
    ...
    // [GC-DIAG] external-linkage accessor for the anon-namespace runtime capture above.
    void* getLastConstraintRuntime(int* outSize)
    {
        if (outSize) { *outSize = g_lastConstraintRuntimeSize; }
        return g_lastConstraintRuntime;
    }
}
