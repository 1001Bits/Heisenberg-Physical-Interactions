...
namespace heisenberg::rock_core::hand_collider_semantics
{
    /*
...
namespace heisenberg::rock_core::hand_bone_collider_geometry_math
{
    /*
    ...
    struct ColliderDimensionLimits
    {
        // Generated colliders are fed by live root-flattened skeleton data, so
        // dimensions are treated as a runtime trust boundary. Bad single-frame
        // skeleton samples and compounded INI scale mistakes must be rejected
        // before Havok shape creation instead of producing oversized bodies.
        float minLength = 0.05f;
        float maxLength = 80.0f;
        float minRadius = 0.01f;
        float maxRadius = 20.0f;
        float maxConvexRadius = 4.0f;
        float maxLongAxisExtent = 120.0f;
    };

    inline bool colliderDimensionsWithinLimits(float lengthValue, float radius, float convexRadius, const ColliderDimensionLimits& limits)
    {
        if (!finitePositive(lengthValue) || !finitePositive(radius) || !std::isfinite(convexRadius) || convexRadius < 0.0f) {
            return false;
        }
        if (lengthValue < limits.minLength || lengthValue > limits.maxLength) {
            return false;
        }
        if (radius < limits.minRadius || radius > limits.maxRadius) {
            return false;
        }
        if (convexRadius > limits.maxConvexRadius) {
            return false;
        }

        const float longAxisExtent = lengthValue + radius * 2.0f;
        return std::isfinite(longAxisExtent) && longAxisExtent <= limits.maxLongAxisExtent;
    }
    ...
    {
        const float half = (std::max)(0.05f, std::isfinite(lengthValue) ? lengthValue : 0.05f) * 0.5f;
        const float r = (std::max)(0.01f, std::isfinite(radius) ? radius : 0.01f);
    ...
    {
        const float hx = (std::max)(0.05f, std::isfinite(lengthValue) ? lengthValue : 0.05f) * 0.5f;
        const float hy = (std::max)(0.05f, std::isfinite(widthValue) ? widthValue : 0.05f) * 0.5f;
        const float hz = (std::max)(0.05f, std::isfinite(thicknessValue) ? thicknessValue : 0.05f) * 0.5f;
        std::vector<Vector> points;
        points.reserve(14);
        ...
        float palmDepth)
    {
        ...
        result.length = (std::max)(1.0f, palmLength);
...
namespace heisenberg::rock_core::hand_bone_grab_pivot_math
{
    /*
