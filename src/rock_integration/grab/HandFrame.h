...
namespace heisenberg::rock_core::handspace_convention
{
    /*
...
namespace heisenberg::rock_core::pointing_direction_math
{
    /*
...
namespace heisenberg::rock_core
{
    ...
    {
        return isLeft ? g_grabCoreConfig.rockLeftGrabLegacyPalmPivotAHandspace : g_grabCoreConfig.rockRightGrabLegacyPalmPivotAHandspace;
    }
    ...
    inline RE::NiPoint3 computePalmNormalFromHandBasis(const RE::NiTransform& handTransform, bool isLeft)
    {
         ...
         */
        const RE::NiPoint3 authoredNormal = g_grabCoreConfig.rockPalmNormalHandspace;
        RE::NiPoint3 normal = transformHandspaceDirection(handTransform, authoredNormal, isLeft);
        if (g_grabCoreConfig.rockReversePalmNormal) {
            normal.x *= -1.0f;
            normal.y *= -1.0f;
            normal.z *= -1.0f;
        }
    ...
    inline RE::NiPoint3 computePointingVectorFromHandBasis(const RE::NiTransform& handTransform, bool isLeft)
    {
        return pointing_direction_math::applyFarGrabNormalReversal(
            transformHandspaceDirection(handTransform, g_grabCoreConfig.rockPointingVectorHandspace, isLeft), g_grabCoreConfig.rockReverseFarGrabNormal);
    }

    // NOTE (Heisenberg): the close/far SELECTION-direction helpers were trimmed here. They pull
    // rock::selection_query_policy from the unported HandSelection module and are NOT used by the
    // grab core — no ported grab header references them (Heisenberg keeps its own viewcaster
    // selection). Re-add them with HandSelection.h if ROCK selection is ever ported.

    inline RE::NiPoint3 computePinchDetectionDirectionFromHandBasis(const RE::NiTransform& handTransform, bool isLeft)
    {
        return transformHandspaceDirection(handTransform, g_grabCoreConfig.rockGrabPinchDetectionDirectionHandspace, isLeft);
    }

    inline RE::NiPoint3 computeGrabAuthorityProxyOffsetLocalGame(bool isLeft)
    {
        return isLeft ? g_grabCoreConfig.rockLeftGrabAuthorityProxyOffsetGameUnits : g_grabCoreConfig.rockRightGrabAuthorityProxyOffsetGameUnits;
    }
