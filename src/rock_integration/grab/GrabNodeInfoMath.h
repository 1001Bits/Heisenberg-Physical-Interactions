...
/*
 * ROCK's authored grab-node diagnostics compute the object-local marker
 * transform that would reproduce a known-good hand/object relationship as
 * ROCK:GrabR/L. Keeping this as pure transform math lets the logger validate the
 * same convention used by authored nodes without coupling tests to runtime Havok
 * or FRIK state.
 */
...
namespace heisenberg::rock_core::grab_node_info_math
{
    ...
    inline EulerDegrees nifskopeMatrixToEulerDegrees(const Matrix& matrix)
    {
        return radiansToDegrees(nifskopeMatrixToEulerRadians<Matrix, EulerDegrees>(matrix));
    }
}
