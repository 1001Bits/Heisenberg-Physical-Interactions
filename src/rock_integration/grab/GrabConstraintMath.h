...
namespace heisenberg::rock_core::grab_constraint_math
{
    /*
    ...
    {
        // SPIN FIX (Jul 6): reproduce HeldBody's PROVEN ragdoll decomposition — the full relative
        // rotation in transformB (frameB) and an IDENTITY motor target (target_bRca). Neither of the
        // old auto-selected modes did this: 'neutralTransformB' put the WHOLE relation into
        // target_bRca (identity transformB) so the ragdoll motor saw a 120° error at t=0 and spun
        // body B a full arc to reach it (the 360° spin); 'relationTransformB' applied the relation
        // TWICE (relation²), only ~ok near identity. HeldBody's native setInBodySpace effectively
        // yields "relation in frameB + identity target" (HeldBodyGrab.cpp:299/1146-1157) → zero motor
        // error at rest → no spin. Emit that unconditionally; the decomposition mode is now moot.
        (void)decompositionMode;
        writeHavokRotationColumns(transformBRotation, proxyInBody.rotate);   // frameB = full relation
        // target_bRca = identity (row-major 3x4): let transformB carry the whole rotation.
        targetBRca[0] = 1.0f; targetBRca[1] = 0.0f; targetBRca[2] = 0.0f;  targetBRca[3] = 0.0f;
        targetBRca[4] = 0.0f; targetBRca[5] = 1.0f; targetBRca[6] = 0.0f;  targetBRca[7] = 0.0f;
        targetBRca[8] = 0.0f; targetBRca[9] = 0.0f; targetBRca[10] = 1.0f; targetBRca[11] = 0.0f;
    }
