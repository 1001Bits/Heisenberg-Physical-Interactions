...
// FULL 1:1 port of ROCK physics-interaction/contact/SoftContactMath.h (784 lines) — the pure
// soft-contact math layer: capsule/point-plane/AABB contact solves, tracked-magnet corrections,
// compliant hard-stop ramp, smoothing, tangent-drift / re-entry sweeps, and the haptic edge.
// Namespace kept as heisenberg::rock_core::soft_contact_math. Only adaptation: bare std::max / std::min /
// numeric_limits calls are parenthesized ((std::max)(...)) because Heisenberg builds without
// NOMINMAX (CommonLibF4's Common.h needs the Windows macros).
//
// ROCK's original header comment:
/*
...
namespace heisenberg::rock_core::soft_contact_math
{
